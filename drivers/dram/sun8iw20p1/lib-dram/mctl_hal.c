...
static inline uint32_t read32(unsigned int addr)
{
        return (*((volatile uint32_t *)(addr)));
}

static inline void write32(unsigned int addr, uint32_t value)
{
...
void auto_set_timing_para(dram_para_t *para) // s5
{
	...
	if (para->dram_tpr13 & 0x2) {
		...
		if (tref == 1) {
			ddr_printf("trefi:3.9ms\r\n");
		} else if (tref == 2) {
			ddr_printf("trefi:1.95ms\r\n");
		} else {
			ddr_printf("trefi:7.8ms\r\n");
		}
	}
...
void mctl_phy_ac_remapping(dram_para_t *para)
{
	...
	ddr_printf("ddr_efuse_type: 0x%x\r\n", fuse);

	val = (unsigned int)(read32(0x03006200) << 0x10) >> 0x18;
	ddr_printf("mark_id: 0x%x\r\n", val);
...
unsigned int mctl_channel_init(unsigned int ch_index, dram_para_t *para)
{
	...
	/*	val = read32(0x3103010);
		if (((val >> 20) & 0xff) && (val & 0x100000)) {
			ddr_printf("ZQ calibration error, check external 240 ohm resistor.\r\n");
			return 0;
		}
	*/
...
int dqs_gate_detect(dram_para_t *para)
{
	...
	if (read32(0x3103010) & (1 << 22)) {
		...
		if (u1 == 2) {
			...
			if (u2 == 2) {
				para->dram_para2 = u1;
				ddr_printf("[AUTO DEBUG] single rank and full DQ!\r\n");
			} else {
				para->dram_para2 = u1 | 1;
				ddr_printf("[AUTO DEBUG] single rank and half DQ!\r\n");
			}
			...
			if (u1 != 0) {
				...
				ddr_printf("DX0 state: %d\r\n", u1);
				ddr_printf("DX1 state: %d\r\n", u2);
				return 0;
			}

			para->dram_para2 = (para->dram_para2 & 0xfffffff0) | 0x1001;
			ddr_printf("[AUTO DEBUG] dual rank and half DQ!\r\n");
		}

	} else {
		para->dram_para2 = (para->dram_para2 & 0xfffffff0) | 0x1000;
		ddr_printf("[AUTO DEBUG] two rank and full DQ!\r\n");
	}
...
int auto_scan_dram_size(dram_para_t *para) // s7
{
	...
	for (rank = 0; rank < maxrank;) {
		...
		if (i > 16)
			i = 16;
		ddr_printf("[AUTO DEBUG] rank %d row = %d\r\n", rank, i);
		...
		banks = (j + 1) << 2; // 4 or 8
#endif
		ddr_printf("[AUTO DEBUG] rank %d bank = %d\r\n", rank, banks);
		...
		ddr_printf("[AUTO DEBUG] rank %d page size = %d KB\r\n", rank, pgsize);
	...
	if (maxrank == 2) {
		...
		if ((rval & 0xffff) == ((rval >> 16) & 0xffff)) {
			ddr_printf("rank1 config same as rank0\r\n");
		} else {
			para->dram_para2 |= 0x00000100;
			ddr_printf("rank1 config different from rank0\r\n");
		}
	}
	return 1;
}
...
signed int init_DRAM(int type, dram_para_t *para) // s0
{
	...
	if (para->dram_tpr13 & 0x10000) {
		ddr_printf("DRAM only have internal ZQ!!\r\n");
	...
	ddr_printf("DRAM BOOT DRIVE INFO: %s\r\n", "V0.24");
	ddr_printf("DRAM CLK = %d MHz\r\n", para->dram_clk);
	ddr_printf("DRAM Type = %d (2:DDR2,3:DDR3)\r\n", para->dram_type);
	if ((para->dram_odt_en & 0x1) == 0) {
		ddr_printf("DRAMC read ODT  off.\r\n");
	} else {
		ddr_printf("DRAMC ZQ value: 0x%x\r\n", para->dram_zq);
	}

	// report ODT
	rc = para->dram_mr1;
	if ((rc & 0x44) == 0) {
		ddr_printf("DRAM ODT off.\r\n");
	} else {
		ddr_printf("DRAM ODT value: 0x%x.\r\n", rc);
	}

	// Init core, final run
	if (mctl_core_init(para) == 0) {
		ddr_printf("DRAM initialisation error : 1 !\r\n");
		return 0;
	}
